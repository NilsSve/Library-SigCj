...
Activate_View Activate_oChartOrderEntryView for oChartOrderEntryView
Object oChartOrderEntryView is a dbView
    Set Border_Style to Border_Thick
    Set Maximize_Icon to True
    Set Label to "Order Entry (Chart)"
    Set Location to 2 3
    Set Size to 174 629
    Set piMinSize to 174 629
    ...
    Object oOrderHea_Order_Total is a dbForm
        ...
        Set Size to 13 60
        Set Location to 156 340
        Set peAnchors to anBottom
        Set Label_Col_Offset to 3
        Set Label_Justification_Mode to jMode_Right
        
        Procedure OnChange
            Send ReLoadAllSeriesPoints of (oVendorChart(oChartOrderEntryView(Self)))
        End_Procedure
    ...
    Object oOrder_Report is a cCrystal
        Set psReportName to "Orders.rpt"

        Property Integer piOrderNumber 0
        ...
        Procedure OnInitializeReport Handle hoReport
            Integer iOrder
            String  sSelection
            Handle hoTerms hoShipVia
            Variant[][2] vTerms
            Variant[][2] vShipVia
        
            Forward Send OnInitializeReport hoReport
        
            Get CreateCDO of hoReport "Terms.ttx" to hoTerms
            If (hoTerms) Begin
                Get TableData of Terms_table to vTerms
                Send AppendCDOData of hoReport hoTerms vTerms
            End
        
            Get CreateCDO of hoReport "ShipVia.ttx" to hoShipVia
            If (hoShipVia) Begin
                Get TableData of Ship_Table to vShipVia
                Send AppendCDOData of hoReport hoShipVia vShipVia
            End
        
            Get piOrderNumber to iOrder
        
            // Set the selection formula to print the one order.
            Move ("{OrderHea.Order_Number} = " + String(iOrder)) to sSelection
        ...
        Procedure PrintOrder Integer iNum
            ...
            Set piOrderNumber to iNum
        
            // check for Crystal
            Get CheckCrystalEnvironment of oCheckForCrystal to bCrystalOK
            If (bCrystalOK) Begin
               Send RunReport
            End
            Else Begin
                Send DisplayDialog of oCheckForCrystal
            End
        
        End_Procedure
        
    End_Object    // oOrder_Report
    ...
    Object oVendorChart is a cSigCJChart
        Set Size to 167 212
        Set Location to 3 415
        Set peAnchors to anAll
        Set pbLegendVisible to False
        Set psTitleText to "Order total by Vendor"
        Set pnTitleFontSize to 11
        Set psPalette to "Photodesign"      // a Palette of colors a series can choose from.
    
        Object oVendorDiagram is a cSigCjDiagram
            
            Object oVendorSeries is a cSigCjSeries
                Set peSeriesStyle to xtpPieSeriesStyle
                Set pbColorEach to True     // when true a different color will be chosen for each seperate series point.
                Set piHolePercent to 25     // the percentage size of the pie which will appear as a hole in the middle.
                Set pbLabelFormatUseThousandSeparator to True
                Set peLabelFormatCategory to xtpChartCurrency

                Procedure OnAddChartData
                    Integer iSize iLoop
                    tdVendorInfo[] taVendorInfo
                    
                    Get RelevantVendors to taVendorInfo
                    Move (SizeOfArray(taVendorInfo)) to iSize
                    For iLoop from 0 to (iSize-1)
                        Send AddPoint taVendorInfo[iLoop].sName taVendorInfo[iLoop].iId taVendorInfo[iLoop].iCount
                    Loop
                End_Procedure
            End_Object             
        End_Object
        
        Procedure OnPointDblClick Short llButton Short llShift Integer iSeriesIndex Integer iPointIndex Handle hoPoint
            Integer iVendorId
            Handle hoDD
            
            Get ComArgumentValueString of hoPoint to iVendorId // retrieve the X-Axis value of the selected point object
            Send Activate_oVendorView
            Move (Vendor_DD(oVendorView(Self))) to hoDD
            Send Clear of hoDD
            Move iVendorId to Vendor.ID
            Send Request_Find of hoDD Eq Vendor.File_Number 1
        End_Procedure
    End_Object
    ...
    Function CompareVendor tdVendorInfo tVendorInfo tdVendorInfo tComparison Returns Integer
        If (tVendorInfo.iId > tComparison.iId) ;
            Function_Return (GT)
        If (tVendorInfo.iId < tComparison.iId) ;
            Function_Return (LT)
        Function_Return (EQ)
    ...
    Function RelevantVendors Returns tdVendorInfo[]
        Integer iId iIndex iSize iAmount iOrder
        tdVendorInfo[] taVendorInfo
        tdVendorInfo   tSearchVendor                   
        
        Get Field_Current_Value of (Server(Self)) Field OrderHea.Order_Number to iOrder
        Clear OrderDtl
        Move iOrder to OrderDtl.Order_Number
        Find Gt OrderDtl by Index.1
        While ((Found) and (OrderDtl.Order_Number=iOrder))
            Relate OrderDtl
            Move Vendor.ID to iId
            Move iId to tSearchVendor.iId
            Move (SearchArray(tSearchVendor,taVendorInfo,Self,(RefFunc(CompareVendor)))) to iIndex
            If (iIndex<>-1) Begin
                Move OrderDtl.Extended_Price to iAmount
                Move (taVendorInfo[iIndex].iCount+iAmount) to taVendorInfo[iIndex].iCount
            End
            Else Begin
                Move (SizeOfArray(taVendorInfo)) to iSize
                Move iId to taVendorInfo[iSize].iId
                Move (Trim(Vendor.Name)) to taVendorInfo[iSize].sName
                Move OrderDtl.Extended_Price to taVendorInfo[iSize].iCount
            End
            Find Gt OrderDtl by Index.1
        Loop
        Send Refind_Records of OrderDtl_DD
        Function_Return taVendorInfo
    End_Function
